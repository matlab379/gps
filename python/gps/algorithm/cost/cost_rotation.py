""" This file defines the rotations cost function. """
import copy

import numpy as np

from gps.algorithm.cost.config import COST_ROT
from gps.algorithm.cost.cost import Cost
from gps.algorithm.cost.cost_utils import get_ramp_multiplier
from gps.proto.gps_pb2 import JOINT_ANGLES, END_EFFECTOR_POINTS, END_EFFECTOR_POINT_JACOBIANS, END_EFFECTOR_POINT_ROT_JACOBIANS, END_EFFECTOR_ROTATIONS_Q
...
class CostRot(Cost):
    ...
    def eval(self, sample):
        ...
        # Choose target.
        tgt = self._hyperparams['target_end_effector_rot']
        pt = sample.get(END_EFFECTOR_ROTATIONS_Q)
        dist = pt - tgt
        # TODO - These should be partially zeros so we're not double
        #        counting.
        #        (see pts_jacobian_only in matlab costinfos code)
        jx = sample.get(END_EFFECTOR_POINT_ROT_JACOBIANS)
