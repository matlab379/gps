...
// #endif
    ...
    // update end effector points target
    Eigen::MatrixXd ee_rotations_q_tgt;
    if( msg->ee_rotations_q_tgt.size()%3 != ee_points.size()%3){
        ROS_ERROR("Got %d ee_rotations_q_tgt (must match ee_points size: %d)",
                (int)msg->ee_rotations_q_tgt.size(), (int)msg->ee_points.size());
    }
    ee_rotations_q_tgt.resize(n_points, 3);
    for(int i=0; i<n_points; i++){
        for(int j=0; j<3; j++){
            ee_rotations_q_tgt(i, j) = msg->ee_rotations_q_tgt[j+3*i];
        }
    }

    sensor_params["ee_rotations_tgt_q"] = ee_rotations_q_tgt;
